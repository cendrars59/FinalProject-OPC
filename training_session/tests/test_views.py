...
def test_render_training_session_list_view(client, user1):
    ...
    path = urls.reverse('training_session-list')
...
def test_render_practice_new_view(client, user1):
    ...
    # we use reverse function to pass the name of the urls to get the full path
    client.force_login(user1)
    path=urls.reverse('training_session-new')
    response = client.get(path)
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_render_training_session_detailed_view(client, user1, training_session1):
    # In onder to avoid the explicit url because it can change
    # we use reverse function to pass the name of the urls to get the full path
    client.force_login(user1)
    path = urls.reverse('training_session-detail', kwargs={'pk':training_session1.pk})
    response = client.get(path)
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_render_training_session_update_view(client, user1, training_session1):
    # In onder to avoid the explicit url because it can change
    # we use reverse function to pass the name of the urls to get the full path
    client.force_login(user1)
    path = urls.reverse('training_session-update', kwargs={'pk':training_session1.pk})
    response = client.get(path)
    assert response.status_code == 200
